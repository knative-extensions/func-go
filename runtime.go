...
var (
	usage   = "run\n\nRuns a Knative Go function handler."
	verbose = flag.Bool("V", false, "Verbose logging [$VERBOSE]")
	port    = flag.Int("port", 8080, "Listen on all interfaces at the given port [$PORT]")
)

// Start accepts a function.Handler and runs it as a cloudevents or HTTP receiver.
// The function handler interface has the following supported signatures.
//
// Handle()
// Handle() error
